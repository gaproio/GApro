...
contract GAPro_Coin // version 1.0
  {
     /* Variables  */
    ...
    event TransferSell(address indexed from, address indexed to, uint256 value, string typex); // only for ico sales
     ...
     /* Init contract with initial supply tokens to the creator of the contract */
    function GAPro_Coin() public
    {
      ...
      balanceOf[owner] = totalSupply;                                           // Give the creator all initial tokens
       name = "GAPro Coin";                                                     // Set the name for display purposes
   ...
   modifier onlyOwner()
    ...
    function tokenDistri (address _tokenAddr, address[] dests, uint256[] values) public onlyOwner returns (uint256) // send token to multiple address
    {
        uint256 i = 0;
